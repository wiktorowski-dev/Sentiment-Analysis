...
class VectorCreator:
    ...
    def transform_text_to_vector(self, data_set_text, data_set_tags_array,
        ...
        if process_glove:
            embedding_matrix, embedding_dict = self.__create_glove_embedding(glove_path, save_glove_post_processing_files,
                                                                   embedding_matrix_file_name,
                                                                   embedding_key_to_val_file_name)
        else:
            embedding_matrix, embedding_dict = self.__load_glove_embedding(embedding_key_to_val_file_name,
                                                                 embedding_matrix_file_name)
        ...
        for sentence in input_text:
            ...
            for word in sentence:
                if word == 0:
                    ...
                    post_word.append(0)
                    continue
                # Get word from the tokenizer
                word_name = tokenizer.index_word[word]

                # Get glove index for the vector from the key equivalent
    ...
    def __create_glove_embedding(glove_path, save_glove_post_processing_files, embedding_matrix_file_name,
                                 embedding_key_to_val_file_name):
        # Set index 0 with the zeros
        embedding_matrix = [np.zeros(100)]
        ...
        with open(glove_path, encoding='UTF-8') as file:
            for line in file:
                ...
                embedding_matrix.append(np.asarray(record, dtype='float32'))
        ...
        if save_glove_post_processing_files and embedding_matrix_file_name and embedding_key_to_val_file_name:
            ...
            np.save(embedding_matrix_file_name, embedding_matrix)

        return embedding_matrix, embeddings_dict
